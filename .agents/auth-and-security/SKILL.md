...
- `USERS` has two fields that distinguish local vs Google users: `provider VARCHAR(50) NOT NULL DEFAULT "Local"` and `google_id TEXT NULL` with `UNIQUE INDEX uq_users_google_id` (`database/parking_db_ddl.sql:44-50`). The set of valid `provider` values lives in `app/features/users/types/users_types.py` as `ProviderType = Literal["Local", "Google", "GitHub"]` with a runtime `VALID_PROVIDERS` frozenset — this is the **single source of truth**, and the repository validates at runtime before any INSERT.
- `AuthService.google_login` (`app/features/auth/services/auth_service.py:91-191`) sets `provider="Google"` and `google_id=user_info.get("sub")` on user creation (`auth_service.py:146-147`). The `sub` is required — if missing, `auth_service.py:131-134` raises `ServiceError("Google no proporcionó un identificador de usuario (sub)")`.
- If a `Local` user tries `google_login` with their existing email, `auth_service.py:126-129` rejects the request to prevent account takeover via email collision. The user must log in with their password or contact an admin to link the accounts.
- **Anti-pattern — do not fix in passing:** `AuthService.recover_password` (`app/features/auth/services/auth_service.py:522-535`) wraps the body in `try/except: pass` and silently swallows all errors. DB outages, Celery outages, malformed emails — every failure is invisible to ops, and the client still sees `"Correo enviado correctamente"`. The function also does not yet check `user.provider` — Google users currently get a "reset password" email they cannot use. Both issues are tracked as follow-ups (anti-enumeration in the response is the security-critical piece; the `provider` check is the UX piece).
